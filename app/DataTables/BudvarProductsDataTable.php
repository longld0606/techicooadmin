...
class BudvarProductsDataTable extends DataTable
{
    /**
    ...
    public function dataTable()
    {
        ...
        $data = BudvarApi::get('/products/findAll', ['name' => $title, 'lang' => $lang, 'category' => $category]);
        return datatables()
            ->collection($data->data)
            ->filter(function () {})
            ->skipPaging()

            ->addColumn('action', 'admin.budvar.products.action')
            // ->addColumn('name', '{{empty($name) ? "" : $name}} ')
            ->addColumn('lang', '{{empty($lang) ? "Vi" : $lang}} ')
            ->addColumn('name', '<a target="_blank" href="https://biabudvar.cz/productss/{{$slug}}">{{empty($name) ? "" : $name}}</a>')
