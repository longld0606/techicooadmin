...
use App\DataTables\BudvarHistoryDataTable;
...
class HistoryController extends AdminController
{
    ...
    public function index(BudvarHistoryDataTable $dataTable)
    {
        return $dataTable->render('admin.budvar.history.index');
    }
    ...
    {
        $data = ['title' => '', 'status' => 'A'];
        return view('admin.budvar.history.item', ['isAction' => 'create', 'item' => $data, 'inputs' => $this->instanceInputs()]);
    }

    protected function storeImage(FormRequest $request)
    {
        $file_str = $request->file('thumb')->store('public/budvar/history');
    ...
    public function store(FormRequest $request)
    {
        ...
        $response = BudvarApi::post('/history/create', $json); 
        if ($response->status == 'success') {
            $ref = $request->get('ref', '');
            if (!empty($ref)) {
                return redirect($ref)->with('success', 'Thêm thông tin History thành công');
            }
            return redirect()->route('admin.budvar.history.index')->with('success', 'Thêm thông tin History thành công');
        }
    ...
    {
        $data = BudvarApi::get('/history/findOne/' . $id);
        $item = $data->data;
        if (empty($item['type'])) $item['type'] = 'BANNER';
        return view('admin.budvar.history.item', ['isAction' => 'show', 'item' =>  $item, 'inputs' => $this->instanceInputs()]);
    }
    ...
    public function edit(string $id)
    {
        ...
        $item = $data->data;
        if (empty($item['type'])) $item['type'] = 'BANNER';
        return view('admin.budvar.history.item', ['isAction' => 'edit', 'item' =>  $item, 'inputs' => $this->instanceInputs()]);
    }
    ...
    public function update(FormRequest $request, string $id)
    {
        ...
        $response = BudvarApi::put('/history/update/' . $id, $json);
        if ($response->status == 'success') {
            $ref = $request->get('ref', '');
            if (!empty($ref)) {
                return redirect($ref)->with('success', 'Chỉnh sửa thông tin History thành công');
            }
            return redirect()->route('admin.budvar.history.index')->with('success', 'Chỉnh sửa thông tin History thành công');
        }
    ...
    {
        $response = BudvarApi::delete('/history/remove/' . $id);
        if ($response->status == 'success') {
